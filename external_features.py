...
def domain_age(domain):
    ...
    try:
        ...
        # The registration date can be a list or a single datetime object
        # If it's a list (multiple values), take the first one
        if isinstance(reg_date, list):
            reg_date = reg_date[0]
        
        # Calculate the domain age
        current_date = datetime.now()
        domain_age = current_date - reg_date
        
        # Return the domain age in years
